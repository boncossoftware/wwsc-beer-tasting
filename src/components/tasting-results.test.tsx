...
test('renders contestant display names correctly', () => {
    const userEmail = 'taster@email.com';
    const results = {
        ...mockResults,
        roundResults: [
            { userEmail: userEmail, totalTaste: 2000 },
        ] as ResultSummary[],
    }
    const users: User[] = [
        { id: userEmail, email: userEmail, displayName: 'Taster' }
    ];

    render(
        <TastingResults results={results} users={users} />
    );

    const contestant = screen.getByText(/Taster/gi);
    expect(contestant).toBeInTheDocument();
});