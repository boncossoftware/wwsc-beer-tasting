...
const EditEvent = () => {
    ...
    const updating = useSelector<RootState, boolean>( s => s?.events?.update.updating );
    const updated = useSelector<RootState, TastingEvent|null>( s => s?.events?.update.updated );
    const error = useSelector<RootState, StoreError|null>( s => s?.events?.update.error );
    const loading = useSelector<RootState, boolean>( s => s?.events?.itemsLoading[id] );
    const item = useSelector<RootState, TastingEvent|undefined>( 
    ...
    const handleSave = () => {
        if (event && window.confirm('All answers and results will be cleared. Are you sure?')) {
