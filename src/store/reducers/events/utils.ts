...
export const propsForEvent = (
  ...
  //Set default values
  event.editingAllowed =
    event.editingAllowed === undefined ? false : event.editingAllowed;
  event.rounds = event.rounds === undefined ? 10 : event.rounds;
  event.asterisksAllowed =
    event.asterisksAllowed === undefined ? 2 : event.asterisksAllowed;
  event.owner = userEmail;
  event.related = related;
  event.tasters = tasters;
  return event;
};
