...
export const ACTION_EVENT_RESULTS_CALCULATE_ERROR =
  "event_results/calculate_error";

export default function calculate(id: string) {
  return async (dispatch: Dispatch<AnyAction>) => {
    dispatch({
      type: ACTION_EVENT_RESULTS_CALCULATING,
      payload: { id, calculating: true },
    });
    try {
      const calculateResults = firebase
        .functions()
        .httpsCallable("calculateResults");
      ...
      dispatch({
        type: ACTION_EVENT_RESULTS_CALCULATE_ERROR,
        payload: { id, error },
      });
    }
    dispatch({
      type: ACTION_EVENT_RESULTS_CALCULATING,
      payload: { id, calculating: false },
    });
  };
}
